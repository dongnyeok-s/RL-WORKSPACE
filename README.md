...
| [`drone_drop_rl/`](drone_drop_rl/) | 3D 드론 투하 시뮬레이션 (Gymnasium 커스텀) | SAC + Shaped vs Sparse 보상 설계 비교 실험 |
...
3. lunar_lander/   — 4대 알고리즘 확장 (DQN, SAC 추가 + 연속 행동 공간)
       ↓
4. drone_drop_rl/  — 3D 드론 투하 시스템 (SAC + 4계층 보상 설계)
```
...
│
└── drone_drop_rl/                     ── 실험 4: 3D 드론 투하 보상 설계 ──
    ├── README.md                      보상 설계 + 알고리즘 제안서
    ├── configs/                       SAC 하이퍼파라미터 (shaped/sparse)
    ├── envs/
    │   ├── drone_sim_3d.py            3D 시뮬레이션 환경 (14D obs, 4D action)
    │   └── rewards.py                 4계층 Shaped + Sparse 보상 함수
    ├── networks/                      GaussianPolicy + TwinQ 네트워크
    ├── agents/                        SAC 에이전트 + ReplayBuffer
    ├── train/
    │   ├── train_sim.py               학습 (--reward shaped/sparse)
    │   └── curriculum.py              커리큘럼 학습 매니저
    ├── evaluate.py                    모델 평가 + Pygame 렌더링
    ├── compare.py                     Shaped vs Sparse 비교 차트 생성
    └── results/                       학습 결과 (모델, 그래프, TensorBoard)
```
...
# 커리큘럼 학습
python -m drone_drop_rl.train.train_sim --curriculum
...
# 드론 투하 RL (Shaped vs Sparse)
tensorboard --logdir drone_drop_rl/results/tensorboard
```
...
### 3D 드론 투하 — Shaped vs Sparse 보상 비교 (SAC, 2M steps)

| | SAC + Sparse | SAC + Shaped |
|---|---|---|
| 보상 구성 | 투하 시 거리 보상만 | 4계층 (안전+효율+접근+정밀도) |
| 총 에피소드 | 127,941 | 64,460 |
| 최종 평균 보상 | 0.92 | **2.29** |
| 평균 착지 거리 | 93.54m | **84.93m** |

> Shaped 보상이 Sparse 대비 약 2.5배 높은 보상을 달성하며,
> 4계층 보상 설계가 학습 효율과 성능 모두에서 우위를 보였다.
...
| [drone_drop_rl/README.md](drone_drop_rl/README.md) | SAC 보상 설계 제안서 + Shaped vs Sparse 비교 |
