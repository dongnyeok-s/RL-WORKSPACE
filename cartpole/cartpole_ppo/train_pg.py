"""
train_pg.py - REINFORCE (Vanilla Policy Gradient) 학습 스크립트

PPO와의 비교를 위해 동일한 총 스텝 수(500K)로 학습한다.
학습 완료 후 results/pg_training_curve.png 에 결과를 저장한다.
...
[예상 결과 비교]
  PPO    : 안정적 상승 곡선, ~300K 스텝에서 수렴
  REINFORCE: 곡선이 크게 흔들리고, 수렴이 느리거나 중간에 무너질 수 있음
"""
...
def save_pg_plot(ep_rewards: list, save_path: str) -> None:
    ...
    PPO 결과와 나란히 비교하기 쉽도록 동일한 형식으로 저장한다.
    """
    ...
    ax.set_title("REINFORCE (Vanilla PG) CartPole-v1 Training Curve")
    ax.legend()
...
def save_comparison_plot(
    ...
    두 알고리즘의 안정성 차이를 시각적으로 확인할 수 있다.
    """
    ...
    for ax, rewards, color, title in [
        ...
        if len(rewards) >= 10:
            ...
            ax.plot(np.arange(window, len(rewards) + 1), avg, color=color, linewidth=2, label="Moving Avg (10)")
        ax.axhline(y=475, color="green", linestyle="--", alpha=0.7, label="Solved (475)")
        ax.set_title(title)
        ax.set_xlabel("Episode")
        ax.set_ylabel("Total Reward")
    ...
    plt.suptitle("REINFORCE vs PPO — CartPole-v1", fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close(fig)
...
def train_pg() -> list:
    ...
    print(f"\n{'='*60}")
    print(f"  REINFORCE (Vanilla PG) CartPole-v1 학습 시작")
    ...
    # ════════════════════════════════════════════════════════════════════════
    # 메인 학습 루프 (에피소드 기반)
    # PPO: 2048 스텝 → 업데이트 / REINFORCE: 에피소드 완주 → 업데이트
    # ════════════════════════════════════════════════════════════════════════
    while timestep < total_timesteps:
        ...
        while not done:
            action = agent.select_action(obs)  # log_prob 내부 저장
            obs, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated
            agent.store_reward(reward)         # reward 내부 저장
            timestep += 1

        # ── ② G_t 계산 + 1번 업데이트 ────────────────────────────────────────
        # PPO와 달리: clip 없음, n_epochs=1, Critic 없음
        result = agent.update()
        ...
        if episode_count % 100 == 0:
            ...
            print(f"  평균 보상 (최근 {CONFIG['reward_window']}개): {mean_reward:7.1f}")
